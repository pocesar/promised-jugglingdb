...
AbstractClass.include = function (objects, include, cb){
  ...
  function processIncludeItem(cls, objs, include, keyVals, objsByKeys){
    var relations = cls.relations, relationName, subInclude;
    ...
    if (!relation) {
      return function (){
        throw new Error('Relation "' + relationName + '" is not defined for ' + cls.modelName + ' model');
      };
    }
    ...
    if (keyVals[relation.keyFrom].length > 0) {
      ...
      return function (cb){
        if (_through) {
          relation.modelThrough.all(req).then(function (objsIncluded){
            return _model.include(objsIncluded, _through);
          }).done(function (throughIncludes){
            done(null, throughIncludes, cb);
          });
        } else {
          relation.modelTo.all(req).done(function (objsIncluded){
            done(null, objsIncluded, cb);
          });
        }
      };
